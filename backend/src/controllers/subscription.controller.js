...
export const createSubscription = async (req, res) => {
    try {
        ...
        if (type === "all-access") {
            if (itemId) {
                return res.status(400).json({ message: "itemId should not be provided for this type of subscription" })
            }
        }
            else {
                if (!itemId) {
                    return res.status(400).json({ message: "itemId should be provided for this type of subscription" })
                }
            }
